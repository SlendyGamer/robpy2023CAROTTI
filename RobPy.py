...
def checa_matriz3x3(m: np.ndarray) -> None:
    """
...
def checa_matriz4x4(m: np.ndarray) -> None:
    """
...
def checa_matriz_rotacao(m3: np.ndarray, det_tol: float = 0.01) -> None:
    ...
    if erro > det_tol:
        raise ValueError('Pelo valor da determinante, a matriz não é de rotação.')
...
def eixo_reta_12(po1: np.ndarray, vs1: np.ndarray, po2: np.ndarray, vs2: np.ndarray, angtol: float=1e-3) -> np.ndarray:
    """
...
def ang_twist_dir_ref_rad(vs1: np.ndarray, vs2: np.ndarray, vref: np.ndarray, projtol: float=1e-3) -> float:
    ...
    if norma_vetor(vref - proj_vetores(vref, pvs) * np.signal(pe)) > projtol:
        raise ValueError('O vetor de referência não possui a orientação apropriada')

    return ang_vetores(vs1, vs2) * np.signal(pe)
